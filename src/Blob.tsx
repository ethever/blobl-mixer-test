...
import {
  /*Shape, ShapeBufferGeometry,*/ WebGLCubeRenderTarget,
  Vector3,
  Quaternion,
  sRGBEncoding,
  DoubleSide,
...
// Assets
import envMapSrc from "./assets/media/envmap.jpg";

// import lavaSrc from '../../../assets/lavatile.jpg'
// import alphaStripesSrc from '../../../assets/alpha-stripes.png'

import gradient0 from "./assets/media/05.png";
import gradient1 from "./assets/media/02.png";
import gradient2 from "./assets/media/03.png";
import gradient3 from "./assets/media/04.png";
import gradient4 from "./assets/media/06.png";
import gradient5 from "./assets/media/07.png";
import gradient6 from "./assets/media/08.png";
import gradient7 from "./assets/media/09.png";
 ...
 */
function Blob({ enableShadow, position, ...props }: any, ref: any) {
  ...
  const isRemix = useUIStore((s: any) => s.isRemix);
  ...
  useControl("Distort", {
    type: "number",
    state: [distort, setDistort as any],
    min: 0.0,
    max: 1.0,
    group: "Blob Noise",
  });
  useControl("Frequency", {
    type: "number",
    state: [frequency, setFrequency as any],
    min: 0.01,
    max: 5,
    group: "Blob Noise",
  });
  ...
  useControl("Pole amount", {
    type: "number",
    state: [gooPoleAmount, setGooPoleAmount as any],
    min: 0,
    max: 1,
    group: "Blob Noise",
  });
  ...
  useControl("Distort", {
    type: "number",
    state: [surfaceDistort, setSurfaceDistort as any],
    min: 0.0,
    max: 10.0,
  ...
  useControl("Frequency", {
    type: "number",
    state: [surfaceFrequency, setSurfaceFrequency as any],
    min: 0.01,
    max: 5,
    group: "Blob Surface Noise",
  });
  useControl("Number of waves", {
    type: "number",
    state: [numberOfWaves, setNumberOfWaves as any],
    min: 0,
    max: 20,
    group: "Blob Surface Noise",
  });
  useControl("Speed", {
    type: "number",
    state: [surfaceSpeed, setSurfaceSpeed as any],
    min: 0.0,
    max: 3,
    group: "Blob Surface Noise",
  });
  useControl("Pole amount", {
    type: "number",
    state: [surfacePoleAmount, setSurfacePoleAmount as any],
    min: 0,
    max: 1,
    group: "Blob Surface Noise",
  });
  ...
  const [blobSegments, setBlobSegments] = useQueryState("segments", 256) as any;
  const [blobScale, setBlobScale] = useQueryState("scale", 1) as any;
  const [fixNormals, setFixNormals] = useQueryState("uv", true) as any;
  const [receiveShadow /*, setReceiveShadow*/] = useQueryState(
    "rshad",
    false
  ) as any;
  const [rotX, setRotX] = useQueryState("rx", 0) as any;
  const [rotY, setRotY] = useQueryState("ry", 0) as any;
  const [rotZ, setRotZ] = useQueryState("rz", 0) as any;
  useControl("Size", {
    type: "number",
    state: [blobScale, setBlobScale as any],
    min: 0.5,
    max: 1.5,
    group: "Blob Geometry",
  });
  useControl("Segments", {
    type: "number",
    state: [blobSegments, setBlobSegments as any],
    min: 1,
    max: 512,
    group: "Blob Geometry",
  });
  useControl("Fix Normals", {
    type: "boolean",
    state: [fixNormals, setFixNormals as any],
    group: "Blob Geometry",
  });
  useControl("Rotate X", {
    type: "number",
    state: [rotX, setRotX as any],
    scrub: true,
    distance: Math.PI / 2,
    group: "Blob Geometry",
  });
  ...
  const blobMatProp = (prop: string) => {
    return (presenceMaterialProps as any)[prop][0];
  };
  ...
  useFrame((s) => {
    ...
    if (grabTarget.current) {
      (grabTarget.current as any).getWorldQuaternion(globalQuaternion);
      (grabTarget.current as any).getWorldPosition(globalPos);
      (blob.current as any).position.copy(globalPos);
      (blob.current as any).quaternion.copy(globalQuaternion);
    }
  ...
  const sceneScale = isRemix
    ? [remixSize, remixSize, remixSize]
    : [gallerySize, gallerySize, gallerySize];
  ...
  useEffect(() => {
    if (envMap) {
      ...
      // (document.querySelector(".loaderblob") as any).style.animation =
      //   "blobhide .5s both 1";
      // (document.querySelector(".loaderprogress") as any).style.display = "none";
    }
  }, [envMap]);
      ...
      <a.group ref={blob} {...props} {...gallerySpring} frustumCulled={false}>
        ...
        >
          ...
          <AnimatedMagicalMaterial
            ref={material}
            map={(blobMatProp("useGradient") ? selectedGradient : null) as any}
